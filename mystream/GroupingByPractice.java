...
public class GroupingByPractice {
  ...
  public static void groupBy() {
    ...
    Map<Integer, List<Employee>> ageToEmployee = employeeList
      .stream()
      .collect(java.util.stream.Collectors.groupingBy(Employee::getAge));
    ageToEmployee.keySet().stream().forEach(key -> {
      System.out.println(key + " " + ageToEmployee.get(key));
    });

    //    Map<String, Double> salaryByDept = employeeList
    //      .stream()
    //      .collect(java.util.stream.Collectors.groupingBy(Employee::getDepartment, Collectors.summingDouble(Employee::getSalary)));
